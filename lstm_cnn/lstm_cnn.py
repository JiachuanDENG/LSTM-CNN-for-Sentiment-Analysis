...
lstm_cnn=LSTM_CNN(20,14,len(vocabulary),1,50,0.5)#embedding_dim,lstm_dim,vocab_size,Ci,Co,dropOut_prob
optimizer=torch.optim.Adam(lstm_cnn.parameters(),0.001)
loss_func=nn.CrossEntropyLoss()

for epoch in range(EPOCH):
    print '**********EPOCH',epoch,'*************'
    for step,(x,y) in enumerate(loader):
        bx=autograd.Variable(x)
